...
| **Linux native host** | Usable end to end since September 2026 — picture, sound, keyboard, mouse, pad, healing without a keyframe, and a **ScreenCast portal route** for a machine that may not read its own scanout (an AppImage): the consent is stored and replayed, so the dialog is one click per *installation* ([ch. 9 §19.15](../design/native-capture-encoder.md)) | What a Linux host still cannot do, in the order a user notices ([§19.16](../design/native-capture-encoder.md)): **(1)** muting the host's own speakers is unwritten — `HostMute` is Windows-only, so `mute_host_audio` is silently ignored and the game plays in the room; the way in is a null PipeWire sink, not a driver; **(2)** the consent's *return* trip (a **new** grant reaching `settings.json`) is written and unit-tested but needs a human click on a real dialog to be seen; **(3)** AV1 is written and ⛔ refused by Mesa 23.2 — reopen on a newer driver; **(4)** the absolute pointer follows the compositor's logical layout on Wayland ([§26](../design/native-capture-encoder.md), issue #18) but no two-screen Linux host has confirmed it yet; **(5)** HDR does not exist at all (`hdrActive = false`, 8-bit capture); **(6)** ⛔ Wayland will never allow reading or moving another client's pointer — by protocol design, not a gap to fill |
...
- Virtual display on Linux Wayland (portal source type *Virtual*): Windows (bundled driver, "MoonlightWeb Virtual Display" installed by the installer, off by default) and macOS (`CGVirtualDisplay`, created by the server itself) ship the display as an app card on the native host: opening it turns it on and makes it primary, the last stream's end turns it off. Linux is studied and next. HDR on the macOS virtual display is to be measured before it is offered. The display is 1080p at the client's own refresh rate (120 Hz when the browser measured none — the same measurement the *Auto* frame rate uses), and "Match my screen" makes it at the client’s own size instead — a phone streams a 2532×1170 desktop no monitor has (Windows: the driver’s mode list is rewritten and the node restarted; macOS: the display is simply created at that size).
