...
- Virtual display on Linux Wayland (portal source type *Virtual*): Windows (driver fetched on demand) and macOS (`CGVirtualDisplay`, created by the server itself) ship *Add Virtual Display* on the native host card; Linux is studied and next. HDR on the macOS virtual display is to be measured before it is offered. Resolution matching on a virtual display (a mode per client) remains.
