...
| `GET /api/native/virtual-display` | 🔓 / 🏠 | Whether a **virtual display** can be added to this machine (`supported`), whether the driver is there (`installed`) and one is up (`active`). An **admin** caller (host machine, or a session that unlocked the remote admin password — LAN or tunnel) also gets `can_install` + `method` (`task` = the installer's elevated task, `elevated` = this process already is, empty = manual), `download_url`, `os_hdr_capable`, `active_displays[]`, `gpus[]`, `presets` and the current `job`. |
| `POST /api/native/virtual-display/add` | 🏠 admin | `{width, height, refresh, hdr, gpu}` — presets only (720p/1080p/1440p/2160p × 60/90/120). Downloads the pinned driver, verifies it, installs it through the elevated helper, sets the mode. 202 + job; 400 outside the presets; 409 when a job runs or nothing here can elevate (`download_url` for the manual path). |
| `POST /api/native/virtual-display/remove` | 🏠 admin | Removes the device node and the driver package. 202 + job. |
