...
| `virtual_display` | object | *(absent)* | **File-only**, written by the app, never seeded. What *Add Virtual Display* put on this machine — `{added, width, height, refresh, hdr, gpu, added_at}` — so the hosts page offers *Remove* and the requested mode is known. Cleared by *Remove virtual display*. Deleting the key does not remove the Windows driver. On macOS this record is what re-creates the display at every start (it lives only as long as the server process). |
...
| `stream_resolution` | string | `"fixed"` | How the streamed size is chosen. `fixed` = the rung in `stream_height`, width derived from the host's shape. `device` = *Same as your device*: this screen's size in physical pixels, so a full-screen picture is shown pixel for pixel — the one choice that may ask a host for more than its display has (the native host then upscales; every other host gets the explicit size). `host` = *Same as the remote PC*: the host display's own size, which only the native host can answer; Sunshine, Apollo and Wolf stream 1080p under it, and the choice is kept for the day the host is native. `custom` = the pair below. Every choice but `fixed` fixes the shape itself: the native host fits its display's shape inside the requested box (a 16:9 display asked for 1920×1200 streams 1920×1080, shown 1:1 with bars on the client) and never exceeds its display except under `device`. |
| `stream_custom_width`, `stream_custom_height` | int | `1920`, `1080` | The `custom` pair, each pinned to 360–4096 and made even at launch. |
| `stream_height` | int | `1080` | 720/1080/1440/2160 — the `fixed` rung. The browser sends `0` to a native host under `host` (its display's own size). |
