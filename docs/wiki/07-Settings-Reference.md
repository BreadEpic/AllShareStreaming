...
| `virtual_display` | object | *(absent)* | **File-only**, written by the app, never seeded. What *Add Virtual Display* put on this machine — `{added, width, height, refresh, hdr, gpu, added_at}` — so the hosts page offers *Remove* and the requested mode is known. Cleared by *Remove virtual display*. Deleting the key does not remove the driver. |
