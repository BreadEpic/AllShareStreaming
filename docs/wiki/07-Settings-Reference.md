...
| `stream_resolution` | string | `"auto"` | How the streamed size is chosen. `auto` = on the native host, the host display's own size when it fits this screen, else the host's picture scaled down on the host, its shape kept, to fit this screen (a 2560×1440 display on a 2000×1600 tablet streams 2000×1125); the stream follows both screens — the host changing its mode, this device turning or moving to another monitor (relaunched at the new size). Sunshine, Apollo and Wolf cannot report their display's size and stream 1080p under it, the width from the measured ratio. `device` = *Match my screen*: this screen's size in physical pixels, pixel for pixel — the native host puts its display in that mode for the session when its driver lists one and upscales otherwise (the one choice that may exceed the host's display); other hosts get the explicit size with GameStream's optimal-settings flag, which lets them switch the host display to it. Follows this screen too. `custom` = the pair below; `fixed` = the rung in `stream_height`, width derived from the host's shape. Every choice but `fixed` fixes the shape itself: the native host fits its display's shape inside the requested box (a 16:9 display asked for 1920×1200 streams 1920×1080, shown 1:1 with bars on the client). |
| `stream_custom_width`, `stream_custom_height` | int | `1920`, `1080` | The `custom` pair, each pinned to 360–4096 and made even at launch. |
| `stream_bitrate_auto` | bool | `true` | The bitrate is the estimate's (resolution × frame rate × HDR/4:4:4) and follows the frame the host really streams — the launch's resolved size, then the native host's answer and its mode changes, moved on the host between two frames (`clientbitrate`). `false` once the slider is moved off the estimate: the value is the user's and stays whatever the resolution becomes; putting the slider back on the estimate makes it automatic again. |
